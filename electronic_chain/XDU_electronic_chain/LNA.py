...
def LNA_get(antennas11_complex_short, N, f0, unit):
    ...
    antenna_Gama_complex = np.zeros((3, N), dtype=complex)
    for p in range(3):
        # Antenna related parameter calculation
        antennas11_short = np.moveaxis(antennas11_complex_short, 0, 1)
        f0 = 1
        f_start = 30
        f_end = 250
    ...
    LNA_Gama_complex = np.zeros((3, N), dtype=complex)  # 3 ports
    LNA_s21_complex = np.zeros((3, N), dtype=complex)
    for p in range(3):
        #  LNA parameter
        str_p = str(p + 1)
