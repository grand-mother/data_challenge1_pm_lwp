...
def main():
    ...
    for i in range(tshower.get_entries()):
        ...
        ex, ey, ez = adjust_traces(np.array(tefield.trace_x), np.array(tefield.trace_y), np.array(tefield.trace_z), Ts)
        ...
        tvoltage.trace_x = v_t[-1][:,0,:].astype(np.float32)
        tvoltage.trace_y = v_t[-1][:,1,:].astype(np.float32)
        tvoltage.trace_z = v_t[-1][:,2,:].astype(np.float32)
