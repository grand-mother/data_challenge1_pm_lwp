    ...
    for in_root_file in clargs.filename:
        ...
        for i in range(tshower.get_entries()):
            ...
            for trace_num in range(len(tefield.trace_x)):
                ...
                # tvoltage.trace_x.append(all_traces_t[-1][0,:].astype(np.float32).tolist())
                # tvoltage.trace_y.append(all_traces_t[-1][1,:].astype(np.float32).tolist())
                # tvoltage.trace_z.append(all_traces_t[-1][2,:].astype(np.float32).tolist())
